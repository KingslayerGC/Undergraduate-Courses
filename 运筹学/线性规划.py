...
c = [-5, 1, -2, 0, 0]
A_eq = [[-2, 1, -1, 1, 0], [2, 1, 1, 0, 1]]
b_eq = [3.5, 2]
result = linprog(c=c, A_eq=A_eq, b_eq=b_eq, method='simplex')

result


# %%
result1 = linprog(c=c, A_eq=A_eq, b_eq=b_eq,
                 bounds=[(0,None), (0,None), (0,None), (0,5), (0,None)],
                 method='simplex')

result1

# %%
result2 = linprog(c=c, A_eq=A_eq, b_eq=b_eq,
                 bounds=[(0,None), (0,None), (0,None), (7,None), (0,None)],
                 method='simplex')
...
import numpy as np
x = np.matrix([0.1, 0.4, 0.5])
A = np.matrix([[10, 5, 2], [8, 9, 11], [4, 7, 12]])
y = np.matrix([0.3, 0.5, 0.2]).T
A*y
...
b_eq = [1]
A_eq = [[1, 1, 1]]
linprog(c=c, A_eq=A_eq, b_eq=b_eq, method='simplex')
